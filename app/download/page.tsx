...
// --- Download Option Card Component (Minor style enhancement) ---
const DownloadOptionCard = ({ icon: Icon, title, description, onClick, colorClass, link, buttonText }) => (
  <motion.div
    whileHover={{ 
      scale: 1.05, 
    ...
    onClick={onClick}
  >
    {/* Intense inner shadow/glow */}
    <div className={`absolute -inset-2 opacity-0 transition-opacity duration-500 group-hover:opacity-10`} 
         style={{ backgroundImage: `radial-gradient(circle at 50% 0, var(--tw-color-${colorClass}-500), transparent)` }} />

    <div className="flex items-start space-x-4">
      <Icon className={`h-8 w-8 text-${colorClass}-400 mt-1 drop-shadow-md`} />
      <div>
...
// --- Main Page Component ---
export default function SexyDownloadingPage() {
  const [selectedOption, setSelectedOption] = useState(null); // 'terrabox' or 'git'
  const [status, setStatus] = useState("Awaiting data retrieval vector selection...");
  ...
  const handleSelectOption = (option) => {
    ...
    if (option === 'terrabox') {
      ...
      // Simulate copying the command
      if (typeof navigator !== 'undefined' && navigator.clipboard) {
        navigator.clipboard.writeText(gitCommand).then(() => {
          setStatus("Git Clone command copied to system clipboard.");
        }).catch(() => {
          setStatus("Error copying command. Click 'COPY COMMAND' manually.");
        });
      } else {
...
//   routing 
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Background Grid and Enhanced Glow Effect */}
      ...
      >
        ...
        {/* Split Footer: Status Tracker and Docs CTA */}
